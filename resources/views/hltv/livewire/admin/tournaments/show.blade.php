<div
    ...
    <div class="flex flex-col gap-4">
                ...
                @if ($tournament->status === 'ongoing')
                    ...
                    @if ($tournament->games->isEmpty())
                        ...
                        <button type="button" wire:click='generateMatchPlan(1)'
                            class="focus:outline-none disabled:opacity-75 disabled:cursor-not-allowed! text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800 cursor-pointer">{{ __('manager.generate_swiss_matchplan') }}</button>
                    @else
        ...
        <div class="bg-gray-800 text-white p-4 rounded shadow">
            <h2 class="text-xl font-semibold mb-2">{{ __('manager.maps') }}
                @if ($tournament->status === 'scheduled')
                    <button type="button" wire:click='addAllAvailableMaps' wire:target='addAllAvailableMaps'
                        wire:loading.attr="disabled" wire:loading.class="opacity-50 cursor-not-allowed! disabled"
                        class="focus:outline-none text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 cursor-pointer"><i
                            class="fa-solid fa-plus" wire:target='addAllAvailableMaps'
                            wire:loading.class.remove='fa-plus' wire:loading.class="fa-spinner fa-spin"></i>
                        {{ __('manager.add_all_maps') }}</button>
                @endif
            </h2>
            <div class="grid grid-cols-12 gap-4">
                @foreach ($availableMaps as $map)
                    ...
                    wire:loading.class="opacity-50 cursor-not-allowed disabled" wire:target='changeMapState()' @endif
                        ...
                        <a>
                            <img class="rounded-t-lg" src="{{ $map->getImageUrlAttribute() }}"
                                alt="{{ $map->name }}" />
                        </a>
                        <div class="p-5">
                            <a>
        ...
        <div class="bg-gray-800 text-white p-4 rounded shadow">
            ...
            @if ($tournament->type === 0) {{-- Bracket style --}}
                ...
                @elseif ($tournament->type === 1)
                    {{-- Swiss elimination style (group phase) --}}
                    @php
                        ...
                        for ($round = 1; $round <= $maxRound; $round++) {
                            ...
                            usort($possibleScores, function ($a, $b) {
                                [$winsA, $lossesA] = explode('-', $a);
                                [$winsB, $lossesB] = explode('-', $b);
                                $winsA = (int) $winsA;
                                $lossesA = (int) $lossesA;
                                $winsB = (int) $winsB;
                                $lossesB = (int) $lossesB;
                                ...
                                if ($isEliminatingA && !$isEliminatingB) {
                                    return 1;
                                }
                                if (!$isEliminatingA && $isEliminatingB) {
                                    return -1;
                    ...
                    <div class="overflow-x-auto">
                        <div class="relative min-w-max pb-4" id="swiss-bracket">
                            <svg id="swiss-lines" class="absolute inset-0 w-full h-full pointer-events-none"
                                style="z-index: 0;"></svg>

                            {{-- Advancing/Eliminating Overview Boxes at the Top --}}
                            <div class="flex justify-end gap-4 mb-4 relative" style="z-index: 2;">
                                <div
                                    class="bg-green-900 bg-opacity-30 border-2 border-green-500 rounded-lg px-4 py-2 flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm text-white">
                                            @php
                                                $advancingCount = collect($teamRecords)
                                                    ->filter(fn($r) => $r['wins'] >= 3)
                                                    ->count();
                                            @endphp
                                ...
                                <div
                                    class="bg-red-900 bg-opacity-30 border-2 border-red-500 rounded-lg px-4 py-2 flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm text-white">
                                            @php
                                                $eliminatedCount = collect($teamRecords)
                                                    ->filter(fn($r) => $r['losses'] >= 3)
                                                    ->count();
                                            @endphp
                                    ...
                                    <div class="flex flex-col gap-1 w-48 flex-1">
                                        ...
                                        <div class="flex flex-col gap-3 items-stretch justify-around flex-1">
                                            ...
                                            @if (count($roundData['advancing_scores'] ?? []) > 0)
                                                <div
                                                    class="border-2 border-green-600 rounded-lg p-2 bg-green-900 bg-opacity-10">
                                                    <div class="text-center mb-2">
                                                        <span
                                                            class="text-xs font-bold text-green-400 uppercase">Advancing</span>
                                                    </div>
                                                    @foreach ($roundData['advancing_scores'] as $scoreLabel)
                                                        @php
                                                            $groupData = $roundData['groups'][$scoreLabel] ?? [
                                                                'games' => [],
                                                                'teams' => [],
                                                                'tbd_slots' => 0,
                                                            ];
                                                        ...
                                                        @if ($hasContent)
                                                            <div class="mb-2">
                                                                <div class="text-center mb-1">
                                                                    <div
                                                                        class="inline-block px-2 py-0.5 rounded-full text-xs font-bold text-white bg-green-700 border border-green-500">
                                                                        {{ $scoreLabel }}
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div class="flex flex-col gap-1">
                                                                    @foreach ($teamsInGroup as $record)
                                                                        <div
                                                                            class="rounded p-1.5 border-2 bg-green-900 bg-opacity-30 border-green-500">
                                                                            <div
                                                                                class="flex items-center justify-between">
                                                                                <div class="flex items-center gap-2">
                                                                                    <div
                                                                                        class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold bg-green-700 border border-green-500">
                                                                                        {{ substr($record['team']->tag, 0, 2) }}
                                                                                    </div>
                                                                                    <span
                                                                                        class="text-xs font-medium text-white">{{ $record['team']->tag }}</span>
                                                                                </div>
                                                                                <span
                                                                                    class="text-xs font-bold text-green-400">✓</span>
                                                                            </div>
                                                                        </div>
                                                                    @endforeach
                                                                    ...
                                                                    @for ($i = 0; $i < $tbdSlots - count($teamsInGroup); $i++)
                                                                        <div
                                                                            class="rounded p-1.5 border-2 border-dashed bg-green-900 bg-opacity-10 border-green-600">
                                                                            <div
                                                                                class="flex items-center justify-center">
                                                                                <span
                                                                                    class="text-xs text-gray-500 italic">TBD</span>
                                                                            </div>
                                                                        </div>
                                                                    @endfor
                                                                </div>
                                            ...
                                            @foreach ($roundData['active_scores'] ?? [] as $scoreLabel)
                                                ...
                                                @if ($hasContent)
                                                    ...
                                                    <div class="relative">
                                                        <div class="text-center mb-1">
                                                            <div
                                                                class="inline-block px-2 py-0.5 rounded-full text-xs font-bold text-white border bg-gray-700 border-gray-600">
                                                                {{ $scoreLabel }}
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="flex flex-col gap-2">
                                                            ...
                                                            @foreach ($gamesInGroup as $game)
                                                                <div id="swiss-game-{{ $game->id }}"
                                                                    ...
                                                                    {{-- Team 1 --}}
                                                                    <div
                                                                        class="flex items-center justify-between px-2 py-1.5 border-b border-gray-700
                                                                @if ($game->status === 'completed' && $game->winner_team_id && $game->winner_team_id !== $game->team1_id) opacity-40 @endif">
                                                                        <div
                                                                            class="flex items-center gap-2 flex-1 min-w-0">
                                                                            @if ($game->team1)
                                                                                <div
                                                                                    class="w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold border
                                                                            @if ($game->status === 'completed' && $game->winner_team_id === $game->team1_id) bg-green-600 border-green-500 text-white
                                                                            @else
                                                                                bg-gray-700 border-gray-600 @endif">
                                                                                    {{ substr($game->team1->tag, 0, 3) }}
                                                                                </div>
                                                                                <span
                                                                                    class="text-sm font-medium truncate">{{ $game->team1->tag }}</span>
                                                                            @else
                                                                                <span
                                                                                    class="text-sm text-gray-500 italic">TBD</span>
                                                                            @endif
                                                                        </div>
                                                                        <span
                                                                    ...
                                                                    {{-- VS Divider --}}
                                                                    <div
                                                                        class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-gray-900 px-2 py-0.5 rounded text-xs text-gray-500 font-bold border border-gray-700 z-5">
                                                                        VS
                                                                    </div>
                                                                ...
                                                                @if ($game->status === 'completed' && $game->winner_team_id && $game->winner_team_id !== $game->team2_id) opacity-40 @endif">
                                                                        <div
                                                                            ...
                                                                            @if ($game->team2)
                                                                                <div
                                                                                    class="w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold border
                                                                            @if ($game->status === 'completed' && $game->winner_team_id === $game->team2_id) bg-green-600 border-green-500 text-white
                                                                            @else
                                                                                bg-gray-700 border-gray-600 @endif">
                                                                                    {{ substr($game->team2->tag, 0, 3) }}
                                                                                </div>
                                                                                <span
                                                                                    class="text-sm font-medium truncate">{{ $game->team2->tag }}</span>
                                                                            @else
                                                                                <span
                                                                                    class="text-sm text-gray-500 italic">TBD</span>
                                                                            @endif
                                            ...
                                            @if (count($roundData['eliminating_scores'] ?? []) > 0)
                                                <div
                                                    class="border-2 border-red-600 rounded-lg p-2 bg-red-900 bg-opacity-10">
                                                    <div class="text-center mb-2">
                                                        <span
                                                            class="text-xs font-bold text-red-400 uppercase">Eliminated</span>
                                                    </div>
                                                    @foreach ($roundData['eliminating_scores'] as $scoreLabel)
                                                        ...
                                                        @if ($hasContent)
                                                            <div class="mb-2">
                                                                <div class="text-center mb-1">
                                                                    <div
                                                                        class="inline-block px-2 py-0.5 rounded-full text-xs font-bold text-white bg-red-700 border border-red-500">
                                                                        {{ $scoreLabel }}
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div class="flex flex-col gap-1">
                                                                    @foreach ($teamsInGroup as $record)
                                                                        <div
                                                                            class="rounded p-1.5 border-2 bg-red-900 bg-opacity-30 border-red-500">
                                                                            <div
                                                                                class="flex items-center justify-between">
                                                                                <div class="flex items-center gap-2">
                                                                                    <div
                                                                                        class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold bg-red-700 border border-red-500">
                                                                                        {{ substr($record['team']->tag, 0, 2) }}
                                                                                    </div>
                                                                                    <span
                                                                                        class="text-xs font-medium text-white">{{ $record['team']->tag }}</span>
                                                                                ...
                                                                                <span
                                                                                    class="text-xs font-bold text-red-400">✗</span>
                                                                            </div>
                                                                        </div>
                                                                    @endforeach
                                                                    ...
                                                                    @for ($i = 0; $i < $tbdSlots - count($teamsInGroup); $i++)
                                                                        <div
                                                                            class="rounded p-1.5 border-2 border-dashed bg-red-900 bg-opacity-10 border-red-600">
                                                                            <div
                                                                                class="flex items-center justify-center">
                                                                                <span
                                                                                    class="text-xs text-gray-500 italic">TBD</span>
                                    ...
                                    <li class="py-3 sm:py-4">
                                        <div class="flex items-center">
                                            ...
                                            <div class="flex-1 min-w-0 ms-4">
                                                <p
                                                    class="text-sm font-medium text-gray-900 truncate dark:text-blue-400">
                                            ...
                                            <div class="flex items-center mt-2">
                                                <div class="shrink-0 mr-2">
                                                    <img class="w-6 h-6 rounded-full"
                                                        src="{{ $player->steam_avatar }}" alt="avatar">
                                                </div>
                                ...
                                <div class="col-span-2">
                                    @if ($game->status === 'scheduled' && $game->team1 && $game->team2)
                                        <a wire:click='startMatch({{ $game->id }})'
                                            class="block text-center cursor-pointer focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-800 focus:outline-none dark:focus:ring-green-800 w-full">{{ __('manager.start_match') }}</a>
                                        ...
                                        <form class="max-w-sm mx-auto mb-2">
                                            ...
                                            <select id="maps_override" name="maps_override"
                                                wire:change='updateMapsOverride({{ $game->id }})'
                                                wire:model='maps_override'
                                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                                <option @if ($game->maps_override == 0) selected @endif
                                                    value="0">{{ __('manager.tournament_default') }}</option>
                                                <option @if ($game->maps_override == 1) selected @endif
                                                    value="1">{{ __('manager.best_of_1') }}</option>
                                                <option @if ($game->maps_override == 3) selected @endif
                                                    value="3">{{ __('manager.best_of_3') }}</option>
                                                <option @if ($game->maps_override == 5) selected @endif
                                                    value="5">{{ __('manager.best_of_5') }}</option>
                                            </select>
                                        </form>
                                    ...
                                    @elseif($game->status === 'ongoing')
                                        @if ($game->maps->where('status', 'ongoing')->isNotEmpty())
                                            <a wire:click='pauseMatch({{ $game->id }})'
                                                class="block text-center cursor-pointer focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-yellow-600 dark:hover:bg-yellow-800 focus:outline-none dark:focus:ring-yellow-800 w-full">{{ __('manager.pause_match') }}</a>
                                        @elseif($game->maps->where('status', 'paused')->isNotEmpty())
                                            <a wire:click='resumeMatch({{ $game->id }})'
                                                class="block text-center cursor-pointer focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-yellow-600 dark:hover:bg-yellow-800 focus:outline-none dark:focus:ring-yellow-800 w-full">{{ __('manager.resume_match') }}</a>
                                        @endif
                                        <a wire:click='abortMatch({{ $game->id }})'
                                            class="block text-center cursor-pointer focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-800 focus:outline-none dark:focus:ring-red-800 w-full">{{ __('manager.abort_match') }}</a>
                                    @endif
                                    <a href="{{ route('api.matches.config', $game->id) }}" target="_blank"
                                        class="block text-center cursor-pointer focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-600 dark:hover:bg-gray-800 focus:outline-none dark:focus:ring-gray-800 w-full">{{ __('manager.show_config') }}</a>
                                    <a href="{{ route('matches.show', $game->id) }}" target="_blank"
                                        class="block text-center text-white cursor-pointer focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 w-full">{{ __('manager.view_match') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <form wire:submit="update" class="">
                            ...
                            <div class="mb-5">
                                <label for="registration_deadline"
                                    ...
                                    ({{ __('manager.optional') }}) @error('registration_deadline')
                                        ({{ $message }})
                                    @enderror
                                </label>
  ...
  <input id="datepicker-format" datepicker datepicker-format="mm-dd-yyyy" type="text" class="block w-full ps-9 pe-3 py-2.5 bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand px-3 shadow-xs placeholder:text-body" placeholder="Select date">
                            ...
                            <div class="mb-5">
                                <label for="max_teams"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white @error('max_teams') dark:text-red-500! @enderror">{{ __('manager.max_teams') }}
                                    @error('max_teams')
                                        ({{ $message }})
                                    @enderror
                                </label>
                            ...
                            <div class="mb-5">
                                <label for="maps_final_game"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white @error('maps_final_game') dark:text-red-500! @enderror">{{ __('manager.gametype') }}
                                    ({{ __('manager.final') }})
                                    @error('maps_final_game')
                                        ({{ $message }})
                                    @enderror
                                </label>
                            ...
                            <div class="mb-5">
                                <label for="map_rounds"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white @error('map_rounds') dark:text-red-500! @enderror">{{ __('manager.rounds_per_map') }}
                                    @error('map_rounds')
                                        ({{ $message }})
                                    @enderror
                                </label>
                            ...
                            <div class="mb-5">
                                ...
                                <select wire:model.blur='guest_mode' id="guest_mode" disabled
                                ...
                                <p id="helper-text-explanation" class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                    {{ __('manager.guest_mode_not_editable') }}</p>
                            </div>
                    </div>
                    <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                        <button type="submit"
                            class="cursor-pointer text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">{{ __('manager.update_tournament') }}</button>
                        </form>
