    ...
    <div>
        <div class="sm:hidden">
            ...
            <select id="tabs" wire:model='activeTab'
                ...
                <option value="scheduled">{{ __('manager.upcoming_matches') }}</option>
                <option value="ongoing">{{ __('manager.running_matches') }}</option>
                <option value="completed">{{ __('manager.finished_matches') }}</option>
            </select>
        </div>
        <ul class="hidden text-sm font-medium text-center text-gray-500 rounded-lg shadow-sm sm:flex">
            <li class="w-full focus-within:z-10">
                <a wire:click='changeTab("scheduled")'
                    ...
                    aria-current="page">{{ __('manager.upcoming_matches') }} <i class="fas fa-spinner fa-spin" wire:loading wire:target='changeTab("scheduled")'></i></a>
            </li>
            <li class="w-full focus-within:z-10">
                <a wire:click='changeTab("ongoing")'
                    ...
                    __('manager.running_matches') }} <i class="fas fa-spinner fa-spin" wire:loading wire:target='changeTab("ongoing")'></i></a>
            </li>
            <li class="w-full focus-within:z-10">
                <a wire:click='changeTab("completed")'
                    ...
                    __('manager.finished_matches') }} <i class="fas fa-spinner fa-spin" wire:loading wire:target='changeTab("completed")'></i></a>
            </li>
        </ul>
    </div>
    <div>
